...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Tambah Potongan Gaji</h2>
        </div>

        <form action="{{ route('deductions.store') }}" method="POST" id="deductionForm">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required onchange="toggleInputs()"
                        ...
                        <option value="per_day" {{ old('type') == 'per_day' ? 'selected' : '' }}>Per Hari</option>
                        <option value="per_hour" {{ old('type') == 'per_hour' ? 'selected' : '' }}>Per Jam</option>
                        <option value="fixed" {{ old('type') == 'fixed' ? 'selected' : '' }}>Tetap (Nominal)</option>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="auto_calculate" value="1" {{ old('auto_calculate') ? 'checked' : '' }}
