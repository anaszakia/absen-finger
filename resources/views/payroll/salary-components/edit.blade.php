...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Edit Komponen Gaji</h2>
        </div>

        <form action="{{ route('salary-components.update', $salaryComponent) }}" method="POST" id="componentForm">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required onchange="toggleInputs()"
                        ...
                        <option value="fixed" {{ old('type', $salaryComponent->type) === 'fixed' ? 'selected' : '' }}>Nominal Tetap</option>
                        <option value="percentage" {{ old('type', $salaryComponent->type) === 'percentage' ? 'selected' : '' }}>Persentase dari Gaji Pokok</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div id="amount-field" style="display: none;">
                    ...
                    <input type="number" name="amount" value="{{ old('amount', $salaryComponent->amount) }}" min="0" step="1"
                ...
                <div id="percentage-field" style="display: none;">
                    ...
                    <input type="number" name="percentage" value="{{ old('percentage', $salaryComponent->percentage) }}" min="0" max="100" step="0.01"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="Keterangan tambahan (opsional)">{{ old('description', $salaryComponent->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $salaryComponent->is_active) ? 'checked' : '' }}
